...
'''
    ...
    input:  pred -> network output
            gt   -> ground truth
    output: iou  -> iou results
'''
def IoU(pred, gt):
...
def imgEval(rote, model, device, eval_loader):
    ...
    for data, box, label in eval_loader:
        ...
        className.append(c.cpu().detach().numpy().reshape(-1).tolist())
        
        iou.append(IoU(pred, box))

        box_out.append(pred.tolist())
...
if __name__ == "__main__":
    ...
    rote = "/workspace/cv2020/kxtang/tiny_vid" # define the rote path
    classify_loss_train = []
    ...
    parser.add_argument('--test_batch', type=int, default=30, metavar='N',
                        help='input batch size for testing (default: 30)')
    parser.add_argument('--lr', type=float, default=0.00001, metavar='LR',
                        help='learning rate (default: 0.00001)')
    args = parser.parse_args()          # load parameters

    torch.manual_seed(5)  # generate random seeds for shuffle dataset
    ...
    saveTxt('./Res_CEL2_loss_acc.txt', classify_loss_train, regression_loss_train, acc)

    imgEval('./Res_CEL2_eval.txt', model, device, eval_loader)
