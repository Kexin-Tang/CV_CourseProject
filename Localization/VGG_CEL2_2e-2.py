...
def train(args, model, device, train_loader, optim, epoch):
    ...
    for idx, (data, box, label) in enumerate(train_loader):
        ...
        loss = loss_classify + 0.02*loss_regression
        loss.backward()
        ...
        if idx % 5 == 0:
            ...
            loss_regression_list.append(0.02*loss_regression.item())
...
if __name__ == "__main__":
    ...
    saveTxt('./VGG_CEL2_2e-2_loss_acc.txt', classify_loss_train, regression_loss_train, acc)
    
    imgEval('./VGG_CEL2_2e-2_eval.txt', model, device, eval_loader)
